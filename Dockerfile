...
RUN apt-get update && apt-get -y --force-yes install cuda-samples-7.5 && apt-get clean && apt-get -y --force-yes install make && apt-get -y --force-yes install g++ && RUN apt-get -y --force-yes install cuda
